...
try {
  ...
  page.on("request", request => { if (request.method() === "POST" && request.url().endsWith("/api/sales")) checkoutRequests.push(request); });
  await page.goto(`${base}/login`);
  await page.getByLabel("Username", {exact:true}).fill("focusedadmin");
  await page.getByLabel("Password", {exact:true}).fill(password);
  const loginStarted = Date.now();
  await page.getByRole("button", {name:/Sign in/i}).click();
  await page.waitForURL("**/dashboard");
  console.log(`Browser login to dashboard: ${Date.now()-loginStarted}ms (isolated local database)`);
  ...
  assert.equal(checkoutRequests.length, 1, "One checkout submission creates one sale request");
  ...
  await page.waitForFunction(() => new DOMMatrix(getComputedStyle(document.querySelector('.thermal-paper')).transform).m42 < -5);
  const initialY = await page.locator('.thermal-paper').evaluate(el => new DOMMatrix(getComputedStyle(el).transform).m42);
  const bodyBox = await page.locator('.thermal-printer-body').boundingBox();
  ...
  assert.ok(await page.locator('.thermal-paper').evaluate(el => new DOMMatrix(getComputedStyle(el).transform).m42) > initialY, 'Paper translates downward without vertical scaling');
  await printer.screenshot({path:'test-results/thermal-feed-completed.png',animations:'allow'});
  assert.match(await printer.innerText(), /Special Mango/);
  assert.deepEqual(await page.locator('.thermal-printer-body').boundingBox(), bodyBox, 'Printer remains fixed');
  ...
  const earlyPrint = await page.evaluate(() => {
    const doc = new DOMParser().parseFromString(window.__printedReceipt.html,'text/html');
    return {receipts:doc.querySelectorAll('.receipt').length, animatedWrappers:doc.querySelectorAll('.thermal-paper,.thermal-feed-window').length};
  });
  assert.equal(earlyPrint.receipts,1);
  assert.equal(earlyPrint.animatedWrappers,0,'Printed invoice contains no moving wrappers');
  await page.waitForTimeout(8200);
  assert.equal(await printer.getAttribute('data-state'), 'completed', 'Printing occurs only once');
  assert.equal(await page.locator('.thermal-paper').evaluate(el => getComputedStyle(el).transform), 'none');
  assert.equal(await page.getByRole('button', {name:/Replay animation|Pause & view/}).count(),0);
  const fullscreen = await page.locator('dialog[open]').boundingBox();
  assert.equal(fullscreen.width,1280); assert.equal(fullscreen.height,900);
  assert.equal(await page.locator('dialog[open]').evaluate(el=>getComputedStyle(el).borderWidth),'0px');
  assert.equal(await page.locator('.thermal-paper .receipt').innerText(), await page.locator('.receipt-print-source .receipt').first().innerText(), 'Animation uses the actual website receipt');
  await page.screenshot({path:'test-results/thermal-fullscreen-desktop.png'});
  ...
  await User.updateOne({ username: "focusedadmin" }, { $inc: { tokenVersion: 1 } });
  assert.equal((await call("/auth/me")).status, 401, "A new request immediately sees revoked authentication");
  const revokedPage = await fetch(`${base}/pos`, { headers: { Cookie: cookie }, redirect: "manual" });
  assert.equal(revokedPage.status, 307, "Server-rendered page rejects revoked authentication");
  ...
  console.log("PASS: single downward feed, stationary printer, no repeat after 8 seconds, borderless fullscreen, shared website receipt, print actions, reduced motion, long receipts, tablet/mobile layouts, no animation runtime errors.");
